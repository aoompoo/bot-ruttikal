...
settings = {
    ...
    "man2":"",
    "man3":"",
    "message":"บัญชีนี้ได้การป้องกันโดย @รัตติกาลเธอมันสวย ระบบได้ทำการบล็อคคุณอัตโนมัติ",
...
mulai = time.time() 
dangerMessage = ["cleanse","group cleansed.","mulai",".winebot",".kickall","mayhem","kick on","Kick","!kickall","nuke","หห","Kick","หหหหห","หหหหหห","หหหหหหหก","16","14"]
dangerMessage2 = ["หหห"]
...
╠
╠❋►ใช้ให้เกิดประโยชน์นะจ๊ะ
╠❋►ทำร้ายคนอื่นโทษแบนทันทีค่ะ"""
    return myHelp

def listgrup():
    listGrup =  """►คำสั่งตั้งค่า⍣@รัตติกาล⍣
╔══════════════════════┓
...
╠❋►คำสั่งตั้งแทคตั้งเข้าตั้งออกให้เปลี่ยน
╠❋►  ;เป็น:นี้นะค่ะเช่น : สวัสดี
...
def helpkicker():
    helpKicker = """คำสั่งค้นหา⍣@รัตติกาล⍣
╔════════════════════┓
...
def helpsetting():
    helpSetting = """คำสั่งกลุ่ม⍣@รัตติกาล⍣
╔══════════════════┓
╠❋► กลุ่ม ➠ ข้อมูลกลุ่ม
╠❋► ไอดีกลุ่ม ➠ไอดีห้อง
╠❋► ชื่อกลุ่ม ➠ ชื่อห้อง
...
def helptexttospeech():
    helpTextToSpeech =   """❋►คำสั่งเลียนแบบ⍣@รัตติกาล⍣
❇ เลียนแบบ on/off 
❇ เลียนแบบ 
❇ เพิ่มเลียนแบบ (@) 
❇ ลบเลียนแบบ (@)"""
...
def helplanguange():
    helpLanguange =    """คำสั่งล่องหน⍣@รัตติกาล⍣
ℹ!แทค
ℹ!มิด
ℹ!คท"""
...
#===============หาชื่อล่อนหน===============================================================#
                    ...
                    ki.sendMessage(to, "บอท@รัตติกาลเธอมันสวยยังอยู่ค่ะ")
                elif text.lower() == 'เทส':
                    line.sendMessage(to, "ท่าถึง100นั้นเเละความสวย@รัตติกาลกำลังโหลด:▒...0%")
                    ...
                    line.sendMessage(to, " @รัตติกาลสวยนะยังอยู่ค่ะ✔")
...
#==============================================================================#
                    ...
                    bulan = ["มกราคม", "กุมภาพันธ์", "มีนาคม", "เมษายน", "พฤษภาคม", "มิถุเธอน", "กรกฎาคม", "สิงหาคม", "กันยายน", "ตุลาคม", "พฤศจิกายน", "ธันวาคม"]
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                ...
                if msg.text in ["speed","sp","Speed","สปีด","!speed","/speed"]:
                    line.sendText(msg.to,"แรงขนาดนี้ เหมือนความสวยของ@รัตติกาล👍")
                if msg.text in ["กินข้าวยัง","กินข้าวยังทุกคน","กินข้าวกันยัง","กินข้าวกันยังทุกคน","กินข้าวยังค่ะ","กินข้าวยังจ๊ะ","กินข้าวยังค่ะทุกคน"]:
                    line.sendText(msg.to,"ยังเลย รอคนมาป้อนอยู่ค่ะ!!.")
                if msg.text in ["666"]:
                    line.sendText(msg.to,"อาการหนักนะเนี๊ย!!. อิอิ😝😝")				
                if msg.text in dangerMessage:
                    random.choice(Rfu).kickoutFromGroup(receiver,[sender])
                    random.choice(Rfu).sendText(msg.to,"ตรวจพบคำสั่งของบอทลบกลุ่ม  @ruttikal BotSelf จำเป็นต้องนำออกเพื่อความปลอดภัยของสมาชิก")										
        if op.type == 17:
          if settings["Wc"] == True:
                ...
                if RfuCctv['cyduk'][op.param1]==True:
                    if op.param1 in RfuCctv['point']:
                        ...
                        if Name in RfuCctv['sidermem'][op.param1]:
                            ...
                            pref=['สวัสดีคนอ่าน➠','ตร๊ะ!..เอ๋!!..😝➠','แอบทำไมอ่ะคะ..😜➠','เห็นนะ อิอิ..😝➠','แอบทำมัย? มาคุยกันหน่อย➠','แอบแฟนเล่นหราาค่ะ💨➠','รู้นะว่าคัยอ่าน👉➠','เล่นซ่อนแอบหรอคร้าาาาา👻➠','มาคุยกับเค้าหน่อย➠','นั่นแน่ แอบหราาา➠','จร๊ะ!!..เอ๋!..➠']
